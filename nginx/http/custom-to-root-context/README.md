...
# NGINX — custom-to-root-context

NGINX proxy at /app/, app at root /. The proxy strips the /app prefix before forwarding to the backend root — scenario-local template handling the path translation.
...
NGINX template (`nginx/http/custom-to-root-context/default.conf.template`):
