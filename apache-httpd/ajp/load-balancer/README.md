...
# Apache HTTPD (AJP) — load-balancer

Two AJP backends balanced by mod_proxy_balancer with sticky sessions via a ROUTEID cookie that Apache synthesizes from the balancer route. A second ws:// balancer carries WebSocket upgrades, which cannot ride AJP.
...
<Proxy "balancer://application-balancer/">
    BalancerMember "ajp://vaadin-1:8009" route=1 flushpackets=on
    BalancerMember "ajp://vaadin-2:8009" route=2 flushpackets=on
