...
Two backends balanced by mod_proxy_balancer with sticky sessions via a ROUTEID cookie that Apache synthesizes from the balancer route (the backend JSESSIONID carries no route id unless Tomcat jvmRoute is configured).
